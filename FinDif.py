...
def FiniteDiff(x, n, spb, uniform):
    ...
    if uniform:
        ...
        if n % 2 == 0: # If even...
            ...
            Dx[n/2:Nx-n/2,:] = sp.spdiags(coeff.T, range(0,n+1), Nx-n, Nx)
        elif n % 2 == 1: # If odd...
            for j in range(int(-np.floor(n/2))-1,int(np.ceil(n/2))):
                A[:,j+int(np.floor(n/2))+1] = ((j*dx)**range(0,n+1))/factorial(range(0,n+1))
            b = np.zeros(n+1)
            b[1] = 1;
            coeff = nlg.solve(A,b)
            coeff = coeff.conj().transpose()
            coeff = np.tile(coeff, [Nx, 1])
            Dx[int(np.ceil(n/2)):Nx-int(np.ceil(n/2)),:] = sp.spdiags(coeff, range(0,n+1), Nx-n-1, Nx)
    else:
        for i in range(0,Nx):
            if i <= np.ceil(n/2):
                # Deal with boundary issues
                ...
                # If n is odd, then bias to which side has the closest point.
                    ...
                    if abs(x[i+int(np.ceil(n/2))] - x[i]) <= abs(x[i-int(ceil(n/2))] - x[i]):
                        A = np.zeros(n+1,n+1)
                        for j in range(-int(np.floor(n/2))-1,int(np.ceil(n/2))):
                            dx = x[i+j] - x[i]
                            A[:,j+int(np.floor(n/2))+1] = (dx**range(0,n+1))/factorial(range(0,n+1))
                        b = np.zeros(n+1)
                        b[1] = 1
                        coeff = nlg.solve(A,b)
                        coeff = coeff.conj().transpose()
                        Dx[i, i-int(np.floor(n/2)):i+int(np.ceil(n/2))] = coeff
                    else:
                        A = np.zeros([n+1,n+1])
                        for j in range(-int(np.ceil(n/2))-1,int(np.floor(n/2))):
                            dx = x[i+j] - x[i]
                            A[:,j+int(np.ceil(n/2))+1] = (dx**range(0,n))/factorial(range(0,n))
                        b = np.zeros(n+1)
                        b[1] = 1
                        coeff = nlg.solve(A,b)
                        coeff = coeff.conj().tranpose()
                        Dx[i, i-int(np.ceil(n/2)):i+int(np.floor(n/2))] = coeff
    return Dx
"""
